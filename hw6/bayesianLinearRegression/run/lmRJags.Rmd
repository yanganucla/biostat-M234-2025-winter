...
date: "Jan-30-2025"
---
0. The root directory for this project has 3 folders: "data", "src" and "run". The source Rmd file (this file) is in the "run" directory. The "data" directory contains a dataset from the National Cancer Institute's Surveillance, Epidemiology and End Results (SEER) database. We will regress lung cancer rates from the 58 counties in California on the corresponding smoking rates from each county. The "src" directory includes a file "modelJags.txt" that encodes a Bayesian linear regression model in the JAGS language and an R program called "dataPrep.r" to prepare the data for the JAGS model. 
